function [U] = solveHelmholtzVectorizedGPU(elements, omega, gamma, kappa, beta, f, hI, n)

hVec = gpuArray(zeros(n,1));
FVec = gpuArray(zeros(n,1));
...
basisFuncGradients = gpuArray([-1 -1; 1 0; 0 1]);
...
Z_t = reshape(K, 9, size(K,3)) - gpuArray(complex(kappa^2 * M_t));
...
FVec(unique(elements.nodeIndex(:,1))) = f(unique(elements.nodeIndex(:,1)));
