...
for j=1:size(sources,2)
    if(sourcesRadii(j) == 0)
        ...
        f(pcenterIdx) =  (1+1./2.*sourceValues(j))./(massDensity*(speed_of_sound./refractionIndex(j))^2);
    else
        ...
        if abs(sourceValues(j)) > 0
            for i=1:size(elements.points,1)
                if norm(elements.points(i,:) - sources(:,j)',2) < sourcesRadii(j) 
                    f(i) = (1+ 1./2.*sourceValues(j))./(massDensity*(speed_of_sound./refractionIndex(j))^2);
                end
            end
        end
    end
