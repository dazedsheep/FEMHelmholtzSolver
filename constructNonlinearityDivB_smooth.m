function [f] = constructNonlinearityDivB_smooth(elements, massDensity, speed_of_sound, speed_of_sound_phantoms, diffusivityDomain, diffusivityPhantoms, centers, radii, values, sourceValueDomain, convection)
    ...
    if(radii(j) == 0)
        ...
        if abs(values(j)) > 0
            for i=1:size(elements.points,1)
              f(i) = f(i) + ((1 + 1./2.*values(j))./(2.*massDensity.*speed_of_sound_phantoms(j)^2.*diffusivityPhantoms(j))).*regularizedDirac(radii(j), norm(elements.points(i,:) - centers(:,j)',2));
            end
        end
    end
end
