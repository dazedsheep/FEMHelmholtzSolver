...
% compute the normals
boundaryPointsSourceNormals = 1./sqrt(sum(elements.points(boundaryPointsSourceIdx,:).^2,2)).*elements.points(boundaryPointsSourceIdx,:); % our center is (0,0), so -> normalisation suffices
...
laplaceu0(1,:) = amplification.*8;
laplaceu0(2,:) = amplification.*4;
...
if useSolutionAsLinPoint == true
    ...
    M = massmatrix(elements.opoints', elements.tri);
    L = -cotmatrix(elements.opoints', elements.tri);
    u1tt = calcSolution(timeMesh.timeMesh1, u1ttf, omega1);
    u1lap = calcSolution(timeMesh.timeMesh1, u1laplacef,omega1);
    %stiffnessMatrix = sparse(elements.rowK, elements.colK, reshape(elements.K, 9 ,size(elements.K,3)), size(elements.points,1),size(elements.points,1));
    %u1lap = laplacian(u0_1, inv(elements.M) * stiffnessMatrix);
    ...
    %u2lap = laplacian(u0_2, inv(elements.M) * stiffnessMatrix);
    ...
    %u3lap = laplacian(u0_3, inv(elements.M) * stiffnessMatrix);
