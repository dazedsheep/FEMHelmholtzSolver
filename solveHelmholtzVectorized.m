...
A = getTransformationMatrixVec(n1,n2,n3);
B(1,1,:) = A(1,1:m);
B(1,2,:) = A(1,(m+1):2*m);
B(2,1,:) = A(2,1:m);
B(2,2,:) = A(2,(m+1):2*m);
...
FVec(elements.nodeIndex(:,1),1) = f(elements.points(elements.nodeIndex(:,1),1),elements.points(elements.nodeIndex(:,1),2));

% get the neumann/robin boundary values
hVec(elements.bedges(:,1)) = hI(elements.points(elements.bedges(:,1), 1), elements.points(elements.bedges(:,1), 2));
